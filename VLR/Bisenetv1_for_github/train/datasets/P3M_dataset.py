...
class P3M(Dataset):
    ...
    def __getitem__(self, idx) -> dict:
        ...
        b = np.where(np.array(label)>=100,1,0)
        # b8u = b.astype(np.uint8)  
        # gray = cv2.cvtColor(b8u, cv2.COLOR_BGR2GRAY)
        label = Image.fromarray(b.astype(np.uint8))
